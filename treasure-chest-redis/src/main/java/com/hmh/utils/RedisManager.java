...
public class RedisManager {
    ...
    public RedisManager(RedisTemplate<String, Object> redisTemplate, StringRedisTemplate stringRedisTemplate) {
     ...
     * @return 增加后的值
     */
    public Long incrBy1(String key, long increment) {
    ...
    /**
     * 添加多个元素到有序集合中(有序集合是按照元素的score值由小到大进行排列)
     *
     * @param key    有序集合
     * @param values 多个元素值
     * @return 有序集合长度
    ...
    public <T> List<T> queryListByKey(final String keyEnum, final Class<T> clazz) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(resultStr)) {
                ...
                log.info("queryListByKey response:{}", value.toString());
                return value;
            }
        } catch (Exception e) {
            log.error("queryListByKey error:{}", e);
        }
        return null;
    }
    ...
    public boolean insertObject(final Object obj, final String keyEnum, final long timeout) {
        log.info("insertObject request:key={},obj={}", keyEnum, obj.toString());
        try {
            ...
            log.info("insertObject response：{}", result);
            return result;
        } catch (Exception e) {
            log.error("insertObject error:{}", e);
        }
        return false;
    }

    /**
     * 4、存放redis,并设置超时时间为前端传过来的时间
     *
     * @param obj      保存对象
     * @param key      关键字
     * @param timeout  有效期
     * @param timeUnit 超时时间单位(例如TimeUnit.SECONDS，TimeUnit.MINUTES)
     * @return boolean      插入结果
     */
    public boolean insertObject(final Object obj, final String key, final long timeout, final TimeUnit timeUnit) {
        log.info("insertObject request:key={},obj={}", key, obj.toString());
        try {
            final String value = JSONObject.toJSONString(obj);
            boolean result = stringRedisTemplate.execute((RedisCallback<Boolean>) connection -> {
    ...
    public boolean deleteObject(final String keyEnum) {
        log.info("deleteObject request:key={}", keyEnum);
        try {
            ...
            log.info("deleteObject response：{}", result);
            return result > 0;
        } catch (Exception e) {
    ...
    public String queryObjectByKey(final String keyEnum) {
        ...
        try {
            ...
            log.info("queryObjectByKey response:{}", resultStr);
            return resultStr;
        } catch (Exception e) {
            log.error("queryObjectByKey error:{}", e);
        }
        return null;
    }
    ...
    public <T> T queryObjectByKey(final String keyEnum, final Class<T> clazz) {
        ...
        try {
            ...
            log.info("queryObjectByKey response:{}", value.toString());
            return value;
        } catch (Exception e) {
            log.error("queryObjectByKey error:{}", e);
        }
    ...
    //自增
    public Long incrby(String key, long liveTime) {
    ...
    //获取自增数据
    public Long getByAtomicLong(String key) {
        RedisAtomicLong entityIdCounter = new RedisAtomicLong(key, stringRedisTemplate.getConnectionFactory());
        log.info("有效期：{}",entityIdCounter.getExpire());
    ...
    /**
     * 以下是关于zset指令集合的操作
     * zset为有序集合。就是在set的基础上，添加了一个score值。zset的每一个成员都有一个分数与之对应，
     * 并且分数可以重复。score就相当于权重，可以根据score值进行排序展示。
     */

    /**
     * zset--添加元素(有序集合是按照元素的score值由小到大进行排列)
     *
     * @param key
     * @param value
     * @param score
    ...
    public Double incrementScore(String key, String value, double score) {
        try {
            ...
            log.error("incrementScore error:{}", e);
        }
    ...
    /**
     * zset--获取集合中给定区间的元素(start 开始位置，end 结束位置, -1查询所有.按元素的score值由小到大排列)
     *
     * @param key
     * @param start
     * @param end
    ...
    public Set<TypedTuple<String>> rangeWithScores(String key, long start, long end) {
        try {
            ...
            log.error("reverseRangeWithScores error:{}", e);
        }
    ...
    /**
     * zset--获取集合中给定区间的元素(start 开始位置，end 结束位置, -1查询所有.按元素的score值由大到小排列)
     *
     * @param key
     * @param start
     * @param end
     ...
     * @param key
     */
    public String queryNoSerializableRedisValue(String key) {
        try {
            return stringRedisTemplate.opsForValue().get(key);
        } catch (Exception e) {
            log.error("调用redis服务查询key是否存在出错！", e);
        }
        return null;
    }
     ...
     * @param keys
     */
    public Set<String> queryKeys(String prefix) {
        try {
    ...
    /**
     * 自增1,并设置超时时间为前端传过来的时间(序列化)
     * Redis的incr命令将 key中储存的数字值自增1。如果 key不存在，那么 key的值会先被初始化为 0，然后再执行incr操作。
     * @param key 关键字
     * @param timeout 有效期
     * @param timeUnit 超时时间单位(例如TimeUnit.SECONDS，TimeUnit.MINUTES)
     * @return Long      自增1结果
     */
    public Long incrby(final String key, final long timeout, final TimeUnit timeUnit) {
        log.info("Incr one request:{}", key);
        try {
            return stringRedisTemplate.execute(new RedisCallback<Long>() {
                @Override
                public Long doInRedis(RedisConnection connection) throws DataAccessException {
                    byte[] redisKey = stringRedisTemplate.getStringSerializer().serialize(key);
                    Long redisValue = connection.incr(redisKey);// 自增1
                    if (timeout > 0 && redisValue.longValue() == 1) {
                        // 设置超时时间为前端传过来的时间
                        stringRedisTemplate.expire(key, timeout, timeUnit);
                    }
                    return redisValue;
