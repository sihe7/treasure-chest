package king.drug.common.utils.http;
...
public final class OkHttpKit {
	...
	public static void main(String[] args) throws Exception {
		log.info(OkHttpKit.get("https://www.google.com.hk").execute());
	}
				...
				if (instance == null) {
					...
					clientBuilder.cookieJar(new CookieJar() {
						...
						public void saveFromResponse(HttpUrl url, List<Cookie> cookies) {
							...
							tempCookieList.forEach(cookie -> tempCookieMap.put(cookie.name(), cookie));
							// 获取domain下的cookie
							...
							if (domainCookieMap == null) {
								...
								tempCookieMap.forEach((name, cookie) -> domainCookieMap.put(name, cookie));
							}
						}
						...
						public List<Cookie> loadForRequest(HttpUrl url) {
							...
							if (domainCookieMap != null && !domainCookieMap.isEmpty()) {
								domainCookieMap.forEach((name, cookie) -> cookieList.add(cookie));
							}
							return cookieList;
						}
					});
	...
	public OkHttpKit fileListMap(Map<String, List<File>> files) {
		...
		files.forEach((key, fileList) -> {
			fileList.forEach(this::checkFile);
	...
	private Request getRequest() {
		...
		if (GET.equals(this.method)) {
			...
			if (needMultipartBody()) {
				...
				if (this.parameters != null) {
					this.parameters.forEach((key, value) -> multipartBodyBuilder.addFormDataPart(key, value.toString()));
				}
				...
				if (this.parameters != null) {
					// 有参数
					this.parameters.forEach((key, value) -> formBodyBuilder.add(key, value.toString()));
				}
